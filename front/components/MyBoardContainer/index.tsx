...
const MyBoardContainer:FC<IMBC> = ({ BLRevalidate, userData, openBId, setOpenBId, c, setLoading, toast}) => {
	...
	const removeBoard = useCallback(async (name) => {
	...
	const submitEdit = useCallback(async () => {
		if (confirm("Board 정보를 수정하시겠습니까?")) {
			await setLoading(true);
			await axios.post(`/api/editBoard/${c.name}`, {
				name: boardName,
	...
	}, [boardName, boardDescription, backgroundImage.url]);
		...
		<BarContainer key={(c.id)}>
			...
			{ openBId === c.id &&
				<ManageBoard>
					...
					<div className="btn" onClick={() => { setOMM(!openManageMembers); }}>
						Members
					</div>
					{ openManageMembers &&
						<MyBoardMembers
