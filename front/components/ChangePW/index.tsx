...
const ChangePW:FC<ICPW> = ({ value, setValue, toast, BLRevalidate }) => {
	...
	const currentPWCheck = useCallback(async (pw) => {
		await setStatus({...status, loading: true});
		await axios.post(`/api/passwordCheck/${value.name}`, {
			password: pw
		}).then(res => {
			setPW('');
			setCheck(true);
	...
	const newPWSet = useCallback(async (pw, checkPW, flg) => {
		if (pw.length < 8 && flg)
		{
		...
		await axios.post(`/api/changePassword/${value.name}`, {
			password: pw,
			changeFlg: flg
		}).then(res => {
			setStatus({
				...status, loading: false, done: res.data,
			});
		}).catch(e => {
			setStatus({
				...status, error: e.response.data, loading: false,
			});
		});
	}, []);
				...
				:
					!pwCheck ?
						...
						<div>
							...
							<div className="btn" onClick={() => { newPWSet(pw, checkPW, true);}}>
								{ value.is_lock ? "변경하기" : "비밀번호 설정하기" }
							</div>
							{ value.is_lock &&
								<div className="btn" onClick={() => { newPWSet('', '', false);}}>
									공개 상태로 변경하기
								</div>
							}
						</div>
