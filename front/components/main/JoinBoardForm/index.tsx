...
const JoinBoardForm: FC<IJoinBoradForm> = ({ board, userData }) => {
	...
	const onChangeProfileImg = useCallback(
		async (e) => {
			...
			await axios
				.post(`/api/uploadImage?type=profile&name=${board && encodeURIComponent(board.name)}`, imageFormData)
				.then((res) => {
					setPI({
						url: res.data.url,
						loading: false,
	...
	const requestJoin = async () => {
		...
		await axios
			.post(`/api/join/${board && encodeURIComponent(board.name)}`, {
				pw,
				nickname,
	...
	if (!board) return <Redirect to="/main" />;
	if (page < 3) {
		return (
			<>
				<Box
							...
							{profileImage.loading ? (
								...
								<div
									style={{
										background:
											profileImage.url !== ''
												? `url(${profileImage.url}) no-repeat center / cover`
