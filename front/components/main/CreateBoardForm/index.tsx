...
const CreateBoardForm: FC<ICreateBoardForm> = ({ BLRevalidate, username }) => {
	...
	const onChangeProfileImg = useCallback(
		async (e) => {
			...
			await axios
				.post(`/api/uploadImage?type=profile&name=${title}`, imageFormData)
				.then((res) => {
					setPI({
						url: res.data.url,
						loading: false,
	...
	const onChangeBackgroundImg = useCallback(
		async (e) => {
			...
			await axios
				.post(`/api/uploadImage?type=background&name=${title}`, imageFormData)
				.then((res) => {
					setBI({
						url: res.data.url,
						loading: false,
			...
			return axios
				...
				.then((res) => {
					if (res) {
						...
						setAniCN('next');
						setPS(pageState + 1);
					} else setWarn('이미 존재하는 Board name입니다.');
				})
				.catch((e) => {
		...
		<>
			<FormBox>
				...
				<div className={`${pageState === 0 ? aniCN : ''}`}>
					<p>Board Name</p>
					<input type="text" value={title} onChange={OCTitle} placeholder="Board 이름을 정해주세요." />
					<p>Board Description</p>
					<input type="text" value={des} onChange={OCDes} placeholder="Board 설명을 작성해주세요." />
				</div>
				<div className={`${pageState === 1 ? aniCN : ''}`}>
					...
					<input type="number" value={ETime} onChange={OCETime} min="0" max="60" />
				</div>
				...
				<div className={`${pageState === 3 ? aniCN : ''}`}>
					<p>Set board background image</p>
						...
						{backgroundImage.loading ? (
							...
							<div
								style={{
									background:
										backgroundImage.url !== ''
											? `url(${backgroundImage.url}?${backgroundImage.now}) no-repeat center / cover`
						...
						{profileImage.loading ? (
							...
							<div
								style={{
									background:
										profileImage.url !== ''
											? `url(${profileImage.url}?${profileImage.now})`
