...
const AddComponent: FC<boxProps> = ({ x, y, width, height, category, offset }) => {
    ...
    const OCHeader = useCallback((e) => {
        setHeader(e.target.value);
    }, []);

    const SwitchComp = () => {
        if (category === 1)
            return <textarea key="textInput" value={text} onChange={OCText} name="text" />;
        else if (category === 2) {
            const headHeight = height / offset.height;
            return (
                <NoteAddBox src={null}>
                    <InputBox
                        size={headHeight}
                    >
                        <InputArea
                            ...
                            value={header}
                            onChange={OCHeader}
                            placeholder="본문 타이틀을 입력해주세요."
                        />
                    </InputBox>
                    <InputBox
                        size={height - headHeight - 20}
                    >
                        <TextArea
                            key="paragraphInput"
                            value={text}
        ...
        return <ImageAddBox>오른쪽 버튼을 클릭해서 사진을 업로드해주세요.</ImageAddBox>;
    };
