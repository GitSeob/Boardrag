...
import { Container, NickNameIpt, SubmitBtn } from './style';
import { ProfileImageBox } from '@components/main/CreateBoardForm/style';
import { IBM, IBL } from '@typings/datas';
...
const EditMyInfo: FC<IEMI> = ({ myData, boardData, toast, setOpen, setLoading, BLRevalidate }: IEMI) => {
	...
	const onChangeProfileImg = useCallback(
		async (e) => {
			...
			await axios
				.post(`/api/uploadImage?type=profile&name=${encodeURIComponent(boardData.name)}`, imageFormData)
				.then((res) => {
					setUploading({
						url: res.data.url,
						loading: false,
	...
	const onSubmitEdit = useCallback(async () => {
		...
		if (confirm('수정하시겠습니까?')) {
			...
			await axios
				.post(`/api/BoardMember/edit/${encodeURIComponent(boardData.name)}`, {
					username: nickname,
					profile_img: uploading.url,
				})
				.then((res) => {
					setUploading({
						...uploading,
	...
	useEffect(() => {
		if (nickname !== myData?.username || uploading.url !== myData?.profile_img) setUpdated(true);
		else setUpdated(false);
	}, [nickname, uploading.url, myData]);
	...
	return (
		<Container>
				...
				{uploading.loading ? (
					...
					<div
						style={{
							background: `${
								uploading.url !== ''
									? `center / cover no-repeat url(${uploading.url}?${uploading.now})`
									: `linear-gradient(#002534 , #090a0f) no-repeat`
							}`,
						}}
					>
						...
						<input type="file" accept=".jpg, .png" ref={imageInput} onChange={onChangeProfileImg} />
					</div>
				)}
			</ProfileImageBox>
			<NickNameIpt type="text" value={nickname} onChange={OCNN} />
			<SubmitBtn
			...
			<SubmitBtn className="exit" onClick={quitBoard}>
				나가기
			</SubmitBtn>
		</Container>
	);
