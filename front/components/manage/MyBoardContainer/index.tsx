...
import { BackgroundImageBox } from '@components/main/CreateBoardForm/style';
import useInput from '@hooks/useInput';
import MyBoardMembers from '@components/manage/MyBoardMembers';
...
const MyBoardContainer: FC<IMBC> = ({
	...
	const onChangeBackgroundImg = useCallback(
		async (e) => {
			...
			await axios
				.post(`/api/uploadImage?type=background&name=${encodeURIComponent(c.name)}`, imageFormData)
				.then((res) => {
					setBI({
						url: res.data.url,
						loading: false,
	...
	const removeBoard = useCallback(
		async (name) => {
			if (confirm(`정말 ${name} 보드를 삭제하시겠습니까?`)) {
				...
				await axios.delete(`/api/deleteBoard/${encodeURIComponent(name)}`).then(() => {
					setOpenBId(-1);
	...
	const submitEdit = useCallback(async () => {
		if (confirm('Board 정보를 수정하시겠습니까?')) {
			await setLoading(true);
			await axios
				.post(`/api/editBoard/${encodeURIComponent(c.name)}`, {
					name: boardName,
